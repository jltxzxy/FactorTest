...
![Generated](/tree/main/Readme%20pictures/image-1.png)
...
Indtest为FactorTest的Inherited Class
...
要求1.一定只能从本地获取数据！！ 基础数据使用FB.read\_feather(DataPath+xx )提取,注意提取后的形式，
...
2. getSql 从数据库获取信息
3. 时间格式转化
1. toTime 字符串转datetime
2. fromTime datetime格式转为int
4. regress 回归函数 （y,x,con=True)   y是因变量 x是自变量 请确保不存在空值（填充） 且y与x行数一致  con=True 加截距项
5. getTradeDateList 获取交易日 日历 getStockList 获取所有股票序列 getRetData获取收益率数据 getFisicalList 获得财务报表发布序列
6. getIndexComponent 获得指数成分股信息 getIndexComponent(300/500/800/1000/'wind')分别相应的指数  
7. getIndexData获取指数日行情数据
8. setStockPool 设定股票池 setStockPool(DF,DFfilter) 用DFfilter筛选DF矩阵

factorDF=FB.setStockPool(factorDF, FB.getIndexComponent(poolname))

9. 申万行业分类
1. getSWIndustryData 获取申万行业数据
2. addSWIndustry 在DF右侧新增一列申万行业分类
10. 申万板块数据
1. ` `getSWSector 获取申万板块数据
2. addSWSector 在DF右侧新增一列申万板块数据
11. getIndRetData 获取行业收益数据DF三列式
12. industryAggregate 合并成为大类行业
13. getBarraData 获取Barra数据（time ,code,十个因子）
14. factorStack 读取矩阵型，并转为（time,code,factorname)
15. Kickout 剔除ST、上市60天以内、停牌股
16. zscore&zscorefac  标准化 dePCT deSTD deMAD 对应三种去极值方法 fillmean fillmedian 对应填充空值

方法：factor=factor.groupby('time').apply(deMAD)

`   `也可以直接用factorInit(factor)定义

17. dayToMonth(DF,factor\_list='',method='last') 日频到月频转换   method:{'last','mean','sum'} 
18. isinGroupT 判断各股票的指标在多空分中的哪一组
19. isinTopK 判断各股票的指标知否在前k, 1表示在，0表示不在
20. calcGroupRet 计算组合收益
21. calcWRPL 计算胜率赔率
22. calcResid 计算回归残差
23. getCMV 获取市值数据
24. getZXIndustryData 获取中信行业分类数据
25. 中性化用 ，月频数据  
1. addXSWindDum 加入SWind列 
2. addXZXind 加入中信行业分类列
3. addXSize 加入对数流通市值列
4. addXBarra 加入barra因子值列
26. 计算行业市值中性化
1. ` `Regbysize 计算现有因子和行业市值因子的回归残差
2. calcNeuIndsize计算纯因子，排除行业市值对因子的影响
27. 计算市值中性化
1. RegbySize 计算现有因子和行业市值因子的回归残差，用于中性化
2. calcNeuSize 计算市值中性，排除市值对因子的影响
28. 计算Barra因子中性化
1. RegbyBarra 计算现有因子和barra因子的回归残差，用于中性化
2. calcNeuBarra 计算Barra因子中性，排除barra因子对待处理因子的影响
3. 用法示例： factorDF=FB.calcNeuIndSize(factorDF, 'ROE\_q')
29. calcIC 用pearsonr和spearmanr方法计算相关性（IC、rankIC）
30. calcIClist 计算组合IC值
31. ZipLocalFiles(file\_path,save\_path,zipname='',t=5)  将file\_path的文件压缩到save\_path 中，分为t组，组名为zipname(str) 默认为zipname+日期
32. copyFile 复制本地文件，需输入文件地址和粘贴地址
33. copyFiles 复制文件夹
34. 计算年成长率
1. \_calcGrowthRate 利用最小二乘法回归计算年成长率
2. calcGrowthRate 利用a中函数计算过去windows年复合变化率（同比）
35. calcAllFactorAns 计算所有储存在factorInfo文件中的因子的收益、IC等信息
36. calcPortfolioRet 计算组合收益率
37. calcAnnualTurnover 获取年度换手率信息
38. DataRenewTime 更新数据库 FactorRenewTime 更新因子库
39. getUpdateStartTime 获得开始更新时间
40. readLocalData 读取本地数据并转为三列式  readLocalDataSet批量读取一系列数据并转为[time,code,xx,xx ]格式 readLocalFeather   读取本地feather文件
1. 注意：这一步会比较慢，如果不是必要建议还是多用矩阵运算
41. 存储模块
1. saveSqlData 存储sql型数据
2. saveDailyData存储日频数据
3. saveFinData用于存储财务数据
4. saveIndData用于存储行业成分股数据
5. saveIndexComponentData存储指数成分股
42. toLongForm 矩阵形式转换为Sql形式数据（三列）

`  `toShortForm Sql形式数据转换为矩阵形式数据

43. fillFisicalMonth 将财务数据（季）因子化【压缩成矩阵形式、拓展到月频、向上填补后再展开】
44. ` `partition 将列表切片再合成为每部分固定长度的嵌套列表
45. transData 将财务数据转化为时间、股票代码、因子名称三列dataframe或矩阵

`  `将更名为transFisicalData

46. calc\_plot 对dataframe进行plot画图
47. 财务数据处理
1. calcFisicalLYR 将3、6、9月末财务数据时间统一为上年末数据
2. calcFisicalq 将6、9、12月末的当年累积时段财务数据转化为当季度内的数据
3. calcFisicalttm 将季度时段财务数据转化为过去四个季度的和（ttm数据）
48. applyindex 直接对DataFrame或Series的index执行func
49. calc\_exp\_list 根据因子半衰期生成权重序列
50. 计算加权数据
1. calcWeightedStd 计算加权标准差
2. calcWeightedMean 计算加权平均
51. rollingRegress 待完善
52. monthToDay 将月频数据dataframe转化为日频数据dataframe，空缺数据向下补齐
53. plotICList 对多空组合的IC和rank IC进行绘图
54. plotPortfolioList 对t分组的每组和多空组合的收益画图
